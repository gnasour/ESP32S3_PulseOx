...
bool MAX_init(i2c_master_bus_handle_t *bus_handle, i2c_master_dev_handle_t *dev_handle)
{
  ...
  // Make handle visible to all functions requiring I2C communication
  g_dev_handle = (i2c_master_dev_handle_t) *dev_handle;
...
uint16_t check(void)
{
  ...
  if (readPointer != writePointer)
  {
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      i2c_read_burst_blocking(g_dev_handle, MAX30102_FIFODATA, readByteStream, toGet);
...
{
  uint8_t data = 0;
  ESP_ERROR_CHECK(i2c_master_transmit_receive(g_dev_handle, &reg, 1, &data, 1, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS));
  return data;
}
